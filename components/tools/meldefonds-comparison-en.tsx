...
export function MeldefondsComparison() {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <InputField
          ...
          onChange={setInitial}
          suffix="€"
          info="Your one-time starting capital at the beginning of the investment period."
        />
        <InputField
          ...
          onChange={setMonthly}
          suffix="€"
          info="Amount you invest additionally every month (savings plan)."
        />
        <InputField
          ...
          suffix="%"
          info="Assumed annual gross return before taxes. Historically, a global equity ETF has returned around 7-8% p.a. (including dividends, before inflation)."
        />
        <InputField
          ...
          onChange={setYears}
          suffix="years"
          info="How long you stay invested. The longer, the greater the difference between Meldefonds and non-Meldefonds due to the compounding effect."
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-border border border-border">
        <div className="bg-background p-6 sm:p-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <InfoTooltip text="A fund registered with the OeKB (Austrian reporting fund). The fund company reports tax-relevant income annually. Taxation: 27.5% KESt only on the actual reported deemed distributions (AgE) — the rest of the value increase remains tax-free until sale." />
          </div>
          <p className="text-xs text-muted-foreground mb-4">
            27.5% KESt only on reported AgE (approx. 30% of return)
          </p>
          <div className="space-y-3">
            <ResultRow label="Final value (net)" value={`€ ${fmt(meldefonds.finalValue)}`} />
            <ResultRow label="Total tax paid" value={`€ ${fmt(meldefonds.totalTax)}`} />
            <ResultRow label="Total contributions" value={`€ ${fmt(meldefonds.totalContributions)}`} />
          </div>
        </div>
        ...
        <div className="bg-background p-6 sm:p-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-serif text-lg">Non-Meldefonds</h3>
            <InfoTooltip text="A fund NOT registered with the OeKB (colloquially: 'Schwarzfonds' / black fund). No tax data is reported to Austrian authorities. Consequence: flat-rate taxation per §186 Abs 2 Z 3 InvFG — 27.5% on 90% of the annual NAV increase plus 100% of all distributions. Significantly more expensive than a Meldefonds." />
          </div>
          <p className="text-xs text-muted-foreground mb-4">
            27.5% on 90% of NAV increase or min. 10% of year-end value (§186 InvFG)
          </p>
          <div className="space-y-3">
            <ResultRow label="Final value (net)" value={`€ ${fmt(nonMeldefonds.finalValue)}`} />
            <ResultRow label="Total tax paid" value={`€ ${fmt(nonMeldefonds.totalTax)}`} />
            <ResultRow label="Total contributions" value={`€ ${fmt(nonMeldefonds.totalContributions)}`} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-foreground text-background p-6 text-center">
        <p className="text-sm opacity-80 mb-1">Meldefonds advantage after {yearsNum} years</p>
        <p className="font-mono text-2xl sm:text-3xl font-medium">+ € {fmt(Math.max(0, difference))}</p>
        <p className="text-xs opacity-60 mt-2">Simplified model calculation — actual results depend on fund income, market performance, and individual circumstances.</p>
      </div>

      {/* SVG Chart */}
      {yearsNum > 0 && (
        <div className="border border-border p-4 sm:p-6">
          <p className="text-xs text-muted-foreground uppercase tracking-wider mb-4">Growth trajectory (net after tax)</p>
...
function GrowthChart({
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto" aria-label="Growth trajectory Meldefonds vs Non-Meldefonds">
      {/* Y-axis labels */}
      {yTicks.map((tick, i) => (
        <g key={i}>
      ...
      <text x={padding.left + 145} y={16} className="fill-current opacity-60" fontSize="10">Non-Meldefonds</text>
    </svg>
  );
}
